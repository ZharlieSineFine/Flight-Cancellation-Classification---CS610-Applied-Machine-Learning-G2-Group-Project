...
.
├── dataset/                          # Raw data
│   ├── flights_with_weather.parquet  # Main dataset (Git LFS, ~128 MB)
│   └── airport_weather_complete.csv  # Weather lookup table
│
├── train_val_test_sets/              # Preprocessed splits (ready-to-use)
│   ├── X_{train,val,test}_with_covid.parquet
│   ├── X_{train,val,test}_no_covid.parquet
│   ├── y_{train,val,test}.parquet
│   └── model_comparison.csv          # Final results table
│
├── model_weights/                    # Trained model artifacts
│   ├── decision_tree_{with,no}_covid.pkl
│   ├── logistic_regression_{with,no}_covid.pkl
│   ├── xgboost_{with,no}_covid.pkl
│   ├── nn_{with,no}_covid.keras
│   └── random_forest_{with,no}_covid.pkl (if present)
│
├── parameter_search/                 # Hyperparameter tuning notebooks
│   ├── hongyuan_neural_network.ipynb # MLP grid search (10 configs × 2 versions)
│   ├── amelia_random_forest.ipynb    # RF RandomizedSearchCV
│   └── malik_xgboost.ipynb           # XGBoost RandomizedSearchCV
│
├── Charlie/                          # Data pipeline & final evaluation
│   ├── charlie_data_pipeline.ipynb   # End-to-end preprocessing
│   └── charlie_final_evaluation.ipynb# Unified model comparison + SHAP
│
├── Saelin/                           # EDA & baseline models
│   ├── saelin_eda.ipynb              # Comprehensive EDA
│   └── saelin_models_update.ipynb    # Route baseline, LR, Decision Tree
│
├── Amelia/                           # Random Forest
│   └── random_forest_combined.ipynb  # RF training & tuning
│
├── Hong Yuan/                        # Neural Network (MLP)
│   ├── MLP_Training_and_Evaluation_(with_is_covid_column).ipynb
│   └── MLP_Training_and_Evaluation_(without_is_covid_column).ipynb
│
├── Malik/                            # XGBoost
│   ├── AML project XGBoost.ipynb                # No-COVID experiments
│   ├── AML project XGBoost with Covid.ipynb     # With-COVID experiments
│   ├── xgboost_no_covid_experiment_log.csv      # Experiment tracker
│   └── xgboost_with_covid_experiment_log.csv    # Experiment tracker
│
├── dataset_how_to_use.md             # Quick-start guide for preprocessed data
├── LICENSE                           # Apache 2.0
└── README.md                         # This file
```
